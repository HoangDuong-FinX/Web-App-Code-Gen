...
export default function TdSuccessScreen({ navigate, state }: ScreenProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 p-8">
      ...
      <p className="font-semibold text-center">{t("tdSuccess.refCode", { code: booking.referenceCode ?? "" })}</p>
      ...
      <button data-testid="back-to-car" aria-label={t("tdSuccess.backToCar")} onClick={() => navigate("car-detail")} className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium">
        {t("tdSuccess.backToCar")}
      </button>
      <button data-testid="back-to-home" aria-label={t("tdSuccess.backToHome")} onClick={() => navigate("home")} className="w-full border border-gray-300 text-gray-700 py-3 rounded-lg font-medium">
