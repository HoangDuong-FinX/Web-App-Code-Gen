...
export default function TdSelectDatetimeScreen({
  navigate,
  goBack,
  setTestDriveBooking,
  state,
  ...
  const timeSlots = useMemo(
    () =>
      selectedDate && booking.showroomId
        ? getTimeSlotsForDate(booking.showroomId, selectedDate)
        : [],
    [selectedDate, booking.showroomId],
  );
  ...
  return (
    <div className="flex flex-col min-h-screen">
          ...
          <div key={step} className="flex items-center gap-2">
            <span
              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                idx <= 1 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-500"
              }`}
            >
              {idx + 1}
            </span>
            <span className={`text-xs ${idx <= 1 ? "font-semibold text-blue-600" : "text-gray-500"}`}>
              {step}
            </span>
      ...
      <div className="p-4 mt-auto">
        <button
          data-testid="datetime-continue"
          aria-label={t("tdDatetime.continue")}
