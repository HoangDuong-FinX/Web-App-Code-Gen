    ...
    <div className="flex flex-col min-h-screen pb-14">
      ...
      <div className="flex flex-col gap-4 p-4">
        {promotions.map((promo) => (
          <button key={promo.id} aria-label={promo.title} data-testid="promo-list-card" onClick={() => setCurrentPromo(promo.id)} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden text-left">
            ...
            <div className="p-3 flex flex-col gap-1">
              ...
              <span className="text-xs text-gray-500">{t("common.applicableFor", { models: promo.applicableModelsPreview })}</span>
            </div>
          </button>
        ))}
      </div>
