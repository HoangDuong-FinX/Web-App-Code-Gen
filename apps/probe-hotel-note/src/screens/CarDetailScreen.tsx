  ...
  return (
    <div className="flex flex-col min-h-screen pb-4">
      <div className="flex items-center p-4 gap-3">
        ...
        <button data-testid="share-action" aria-label={t("carDetail.share")} onClick={() => {}} className="p-2 text-gray-600">
          {"\uD83D\uDD17"}
        </button>
      </div>
      ...
      <div aria-label={car.name} className="overflow-hidden">
        <div className="flex overflow-x-auto snap-x snap-mandatory">
          {car.photos.map((photo, idx) => (
            <img key={idx} src={photo.url} alt={photo.label} className="flex-shrink-0 w-full aspect-video object-cover snap-center" />
          ))}
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        {car.hasActivePromo && (
          <span className="inline-block px-2 py-0.5 rounded text-xs font-medium w-fit bg-amber-100 text-amber-700" aria-label={t("carDetail.promo.label", { label: car.promoLabel })}>
            {car.promoLabel}
          </span>
        )}
      ...
      <section className="p-4">
        ...
        <div className="mt-2 flex flex-col gap-1">
          ...
          <a href={`tel:${car.dealer.phone}`} data-testid="dealer-phone" aria-label={car.dealer.name} className="text-blue-600 text-sm font-medium">
            {car.dealer.phone}
          </a>
        </div>
      </section>
      ...
      <div className="p-4 flex flex-col gap-3">
        <button data-testid="inquiry-cta" aria-label={t("carDetail.inquiry")} onClick={handleInquiry} className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium">
          {t("carDetail.inquiry")}
        </button>
        <button data-testid="test-drive-cta" aria-label={t("carDetail.testDrive")} onClick={handleTestDrive} className="w-full border border-blue-600 text-blue-600 py-3 rounded-lg font-medium">
          {t("carDetail.testDrive")}
        </button>
        {car.status === "available" && (
          <button data-testid="reserve-cta" aria-label={t("carDetail.reserve")} onClick={handleReserve} className="w-full border border-blue-600 text-blue-600 py-3 rounded-lg font-medium">
            {t("carDetail.reserve")}
          </button>
        )}
        <button
          ...
          className={`w-full py-3 rounded-lg font-medium ${state.compareList.includes(car.id) ? "bg-blue-100 text-blue-600" : "text-gray-600 bg-gray-50"}`}
