...
export default function ActivityDetailScreen({ goBack, setCurrentCar, state }: ScreenProps) {
  const activity = state.currentActivityId ? getActivityById(state.currentActivityId) : undefined;
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="p-4">
        ...
        <div className="flex items-center gap-3 mt-2">
          <img src={activity.carThumbnailUrl} alt={activity.carName} className="w-20 aspect-[4/3] object-cover rounded-lg" />
          <div>
      ...
      <section className="p-4">
        ...
        <span className={`inline-block mt-2 px-3 py-1 rounded text-sm font-medium ${activity.statusVariant === "success" ? "bg-green-100 text-green-700" : activity.statusVariant === "warning" ? "bg-amber-100 text-amber-700" : activity.statusVariant === "error" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"}`} aria-label={t("common.status", { status: activity.statusLabel })}>
      ...
      <section className="p-4">
        ...
        <div className="mt-2">
          {activity.timeline.map((event: { date: string; description: string }, idx: number) => (
      ...
      <div className="p-4">
        <button data-testid="view-car" aria-label={t("activityDetail.viewCar")} onClick={() => setCurrentCar(activity.carId)} className="w-full border border-blue-600 text-blue-600 py-3 rounded-lg font-medium">
