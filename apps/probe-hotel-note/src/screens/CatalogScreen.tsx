  ...
  return (
    <div className="flex flex-col min-h-screen pb-14">
      <div className="flex items-center p-4 gap-3">
        ...
        <button data-testid="search-trigger" aria-label={t("common.search")} onClick={() => navigate("search")} className="p-2 text-gray-600">
          {"\uD83D\uDD0D"}
        </button>
      </div>
      ...
      <div className="flex gap-2 px-4 py-2 overflow-x-auto">
        {["Sedan", "SUV", "Truck", "Hatchback"].map((type) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap ${activeFilter === type ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}
          >
            {type}
          </button>
        ))}
      ...
      <div className="flex items-center justify-between px-4 py-2">
        ...
        {hasActiveFilters && (
          <button data-testid="clear-filters" aria-label={t("catalog.clearFilters")} onClick={() => setActiveFilter(null)} className="text-blue-600 text-xs font-medium">
            ...
            >
              ...
              <div className="flex-1 p-3 flex flex-col gap-1">
                ...
                <button
                  ...
                  className={`mt-1 text-xs font-medium self-start px-2 py-1 rounded ${state.compareList.includes(car.id) ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-600"}`}
