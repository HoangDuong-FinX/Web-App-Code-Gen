...
export default function MealSelection(props: ScreenProps) {
  const { booking, setBooking, navigate, ancillaryOptions, returnAncillaryOptions } = props;
  const [activeLeg, setActiveLeg] = useState<'outbound' | 'return'>('outbound');
  const [selections, setSelections] = useState<AncillarySelection[]>(booking.ancillarySelections.filter(s => ancillaryOptions.find(o => o.optionId === s.optionId && o.category === 'meal')));

  const options = activeLeg === 'outbound'
    ? ancillaryOptions.filter(o => o.category === 'meal')
    : returnAncillaryOptions.filter(o => o.category === 'meal');
  ...
  const handleConfirm = () => {
    setBooking(prev => activeLeg === 'outbound'
      ? { ...prev, ancillarySelections: [...prev.ancillarySelections.filter(s => !ancillaryOptions.find(o => o.optionId === s.optionId && o.category === 'meal')), ...selections] }
      : { ...prev, returnAncillarySelections: [...prev.returnAncillarySelections.filter(s => !returnAncillaryOptions.find(o => o.optionId === s.optionId && o.category === 'meal')), ...selections] }
    );
  ...
  return (
    <div className="screen">
      <div className="header-row">
        ...
        <Text variant="title-2" as="h1">{t('meal.title')}</Text>
        <div style={{ width: 40 }} />
      </div>

      {booking.tripType === 'round' && (
        <div style={{ display: 'flex', gap: 'var(--gap-008)' }} aria-label={t('meal.tab.aria')}>
      ...
      {options.length === 0 && <Text>{t('meal.noOptions')}</Text>}
      ...
      <Button variant="gradient" onClick={handleConfirm} ariaLabel={t('meal.confirm.aria')}>
        {t('meal.confirm')}
      </Button>
    </div>
  );
