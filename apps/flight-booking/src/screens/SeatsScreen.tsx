...
export function SeatsScreen({ navigate }: SeatsScreenProps) {
  ...
  function handleSeatClick(seat: SeatInfo) {
    ...
    if (existing) { updated = localSeats.map((s) => s.passengerIndex === activePassenger + 1 ? { ...s, seatId: seat.seatId, seatLabel: seat.seatId, price: seat.priceAmount! } : s); } else { updated = [...localSeats, { passengerIndex: activePassenger + 1, seatId: seat.seatId, seatLabel: seat.seatId, price: seat.priceAmount! }]; }
    setLocalSeats(updated);
    const nextUnassigned = state.passengerForms.findIndex((_, i) => i !== activePassenger && !updated.some((s) => s.passengerIndex === i + 1));
  ...
  const legend = Array.from(new Set(seatMap.map((s) => s.fareTier))).map((tier) => { const sample = seatMap.find((s) => s.fareTier === tier && s.priceAmount !== null); return { tier, price: sample?.priceAmount ?? 0 }; });
