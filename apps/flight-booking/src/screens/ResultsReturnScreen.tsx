...
export function ResultsReturnScreen({ navigate }: ResultsReturnScreenProps) {
  ...
  return (
    <div className="p-4 flex flex-col gap-3">
        ...
        {dateChips.map((chip) => (
          <button
            ...
            className={`flex-shrink-0 flex flex-col items-center px-3 py-2 rounded-lg border text-xs transition-colors ${
              chip.date === selectedDate
                ? 'border-red-600 bg-red-50 text-red-700'
                : 'border-gray-200 text-gray-600 hover:bg-gray-50'
            }`}
            onClick={() => setSelectedDate(chip.date)}
            aria-label={`${chip.label}, ${chip.lowestPrice !== null ? formatPrice(chip.lowestPrice) : ''}`}
            data-testid="date-chip"
          >
            <span className="font-medium">{chip.label}</span>
            {chip.lowestPrice !== null && (
              <span className="text-[10px] mt-0.5">{formatPrice(chip.lowestPrice)}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {offers.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-8" data-testid="no-flights-state">
          <span className="text-4xl" aria-hidden="true">&#9992;</span>
          <p className="text-sm text-gray-500">{t.results.noFlights}</p>
        </div>
      ) : (
          ...
          {offers.map((offer) => (
            <div key={offer.offerId} className="border border-gray-200 rounded-lg p-4" data-testid="flight-card">
              <div className="flex justify-between items-center mb-2">
                <span className="font-bold text-sm text-gray-900" data-testid="flight-number">{offer.flightNumber}</span>
                <span className="text-xs text-gray-500" data-testid="duration">{offer.duration}</span>
              </div>
              <div className="flex justify-between mb-3">
                <span className="font-bold text-sm" data-testid="departure-time">{offer.departureTime}</span>
                <span className="font-bold text-sm" data-testid="arrival-time">{offer.arrivalTime}</span>
              </div>
                ...
                {offer.fareClasses.map((fare) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 min-w-0 py-2 px-2 rounded-lg border text-xs font-medium transition-colors ${
                        !fare.available
                      ...
                      onClick={() => handleFareSelect(offer, fare)}
                      aria-label={t.results.farePerPax.replace('{fareClass}', fare.fareClassName).replace('{price}', formatPrice(fare.priceAmount))}
                      data-testid="fare-class-option"
                    >
                      <div>{fare.fareClassName}</div>
                      <div className="text-[10px] mt-0.5">{formatPrice(fare.priceAmount)}</div>
                    </button>
                  );
                })}
              </div>
