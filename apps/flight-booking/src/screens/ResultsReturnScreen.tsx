...
interface ResultsReturnScreenProps { navigate: (screen: ScreenId, extra?: Partial<NavigationState>) => void; }
...
export function ResultsReturnScreen({ navigate }: ResultsReturnScreenProps) {
  ...
  const session = state.returnSession; const offers = session?.offers ?? [];
  const dateChips = useMemo(() => generateDateChips(state.returnDate), [state.returnDate]);
  if (!session) { navigate('search'); return null; }

  function handleFareSelect(offer: FlightOffer, fare: FareClass) { if (!fare.available) return; dispatch({ type: 'SELECT_RETURN_FARE', payload: { offer, fare } }); }
  function handleContinue() { if (!state.selectedReturnFare) return; navigate('passengers'); }
