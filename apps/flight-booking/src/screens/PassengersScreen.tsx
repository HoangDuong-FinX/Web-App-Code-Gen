...
export function PassengersScreen({ navigate }: PassengersScreenProps) {
  ...
  function handleSelfToggle() {
    ...
    if (next && state.passengerForms.length > 0) {
      dispatch({ type: 'UPDATE_PASSENGER', payload: { index: 0, form: { ...state.passengerForms[0], lastName: 'Nguyen', firstName: 'Van A' } } });
    } else if (!next && state.passengerForms.length > 0) {
      dispatch({ type: 'UPDATE_PASSENGER', payload: { index: 0, form: { ...state.passengerForms[0], lastName: '', firstName: '' } } });
    }
  }

  function updateField(index: number, field: keyof PassengerForm, value: string) {
    dispatch({ type: 'UPDATE_PASSENGER', payload: { index, form: { ...state.passengerForms[index], [field]: value } as PassengerForm } });
  }
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    const passengers = state.passengerForms.map((f) => ({ lastName: f.lastName, firstName: f.firstName, gender: f.gender, dateOfBirth: f.dob && /^\d{4}-\d{2}-\d{2}$/.test(f.dob) ? f.dob : null, phone: f.phone || null, email: f.email || null }));
    ...
    const updatedForms = state.passengerForms.map((f, i) => ({ ...f, passengerId: res.data!.passengers[i]?.passenger_id ?? `pax_${i + 1}` }));
    ...
    setLoading(false); navigate('services');
  }

  function getTypeLabel(type: string): string {
    if (type === 'adult') return t.passengers.adult; if (type === 'child') return t.passengers.child; return t.passengers.infant;
  }
  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        <div className={`relative w-10 h-6 rounded-full transition-colors ${selfToggle ? 'bg-red-600' : 'bg-gray-300'}`} onClick={handleSelfToggle} data-testid="self-toggle" aria-label={t.passengers.iAmPassenger} aria-checked={selfToggle}>
          <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${selfToggle ? 'translate-x-4' : 'translate-x-0.5'}`} />
        </div>
