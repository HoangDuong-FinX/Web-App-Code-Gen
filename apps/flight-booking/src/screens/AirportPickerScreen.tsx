    ...
    return state.airports
      .map((group) => ({
        ...
        airports: group.airports.filter(
          (a) =>
            removeDiacritics(a.airportCode).includes(q) ||
            removeDiacritics(a.airportName).includes(q) ||
            removeDiacritics(a.cityName).includes(q)
        ),
      }))
    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col" aria-label={t.airportPicker.heading}>
      <div className="flex justify-between items-center p-4 border-b border-gray-200">
        ...
        <button
          ...
          data-testid="close-airport-picker"
        >
          &#10005;
        </button>
      </div>
      ...
      <div className="p-4">
        <input
          ...
          aria-label={t.airportPicker.searchPlaceholder}
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {filteredGroups.map((group) => (
          <div key={group.groupName} className="mb-4">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2" data-testid="airport-group-header">
              {group.groupName}
            </h2>
            {group.airports.map((airport) => (
              <button
                ...
                aria-label={`${airport.airportCode} - ${airport.airportName}, ${airport.cityName}`}
              >
