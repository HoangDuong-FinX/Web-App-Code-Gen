...
interface SearchScreenProps { navigate: (screen: ScreenId, extra?: Partial<NavigationState>) => void; }
...
export function SearchScreen({ navigate }: SearchScreenProps) {
  ...
  const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [searchError, setSearchError] = useState<string | null>(null);

  useEffect(() => { if (!state.masterDataLoaded) { loadMasterData(); } }, []);

  async function loadMasterData() {
    setError(null);
    const [airportsRes, pairsRes] = await Promise.all([sdk.http.get<AirportGroup[]>('/airports'), sdk.http.get<CityPair[]>('/city-pairs')]);
    if (airportsRes.isSuccess && airportsRes.data && pairsRes.isSuccess && pairsRes.data) {
      dispatch({ type: 'SET_AIRPORTS', payload: airportsRes.data }); dispatch({ type: 'SET_CITY_PAIRS', payload: pairsRes.data }); dispatch({ type: 'SET_MASTER_DATA_LOADED', payload: true });
    } else { setError(t.search.masterDataError); }
  }

  const isRouteValid = useCallback((): boolean => {
    if (!state.origin || !state.destination) return false;
    return state.cityPairs.some((p: CityPair) => p.origin === state.origin?.airportCode && p.destination === state.destination?.airportCode);
  ...
  async function handleSearch() {
    ...
    const outboundRes = await sdk.http.post<{ session_id: string; expires_at: number; offers: SearchSession['offers'] }>('/search', { origin: state.origin.airportCode, destination: state.destination.airportCode, departureDate: state.departureDate, adults: state.adults, children: state.children, infants: state.infants });
    if (!outboundRes.isSuccess || !outboundRes.data) { setSearchError(t.search.error); setLoading(false); return; }
    dispatch({ type: 'SET_OUTBOUND_SESSION', payload: { sessionId: outboundRes.data.session_id, expiresAt: outboundRes.data.expires_at, offers: outboundRes.data.offers } });
    if (state.tripType === 'roundTrip') {
      const returnRes = await sdk.http.post<{ session_id: string; expires_at: number; offers: SearchSession['offers'] }>('/search', { origin: state.destination.airportCode, destination: state.origin.airportCode, departureDate: state.returnDate, adults: state.adults, children: state.children, infants: state.infants });
      if (returnRes.isSuccess && returnRes.data) { dispatch({ type: 'SET_RETURN_SESSION', payload: { sessionId: returnRes.data.session_id, expiresAt: returnRes.data.expires_at, offers: returnRes.data.offers } }); }
    }
    dispatch({ type: 'ADD_RECENT_SEARCH', payload: { tripType: state.tripType, origin: state.origin, destination: state.destination, departureDate: state.departureDate, returnDate: state.returnDate, adults: state.adults, children: state.children, infants: state.infants } });
    setLoading(false); navigate('results');
  }

  const showNoRoute = state.origin && state.destination && !isRouteValid();
  const passengerLabel = t.search.passengerSummary.replace('{adults}', String(state.adults)).replace('{children}', String(state.children)).replace('{infants}', String(state.infants));

  return (
    <div className="p-4 flex flex-col gap-4">
      <h1 className="text-2xl font-bold text-gray-900">{t.search.heading}</h1>
      {error && (<div className="bg-red-50 border border-red-200 rounded-lg p-3" data-testid="master-data-error"><p className="text-red-700 text-sm">{error}</p><button className="mt-2 text-sm text-red-600 font-medium underline" onClick={loadMasterData} aria-label={t.common.retry}>{t.common.retry}</button></div>)}
      ...
      {state.recentSearches.length > 0 && (
        <div className="flex flex-col gap-2 mt-2">
          <div className="flex justify-between items-center"><h2 className="text-lg font-semibold text-gray-900">{t.search.recentSearches}</h2><button className="text-sm text-red-600 font-medium" onClick={() => dispatch({ type: 'CLEAR_RECENT_SEARCHES' })} aria-label={t.search.clearAll} data-testid="clear-all-recent">{t.search.clearAll}</button></div>
          {state.recentSearches.map((rs: import('../store').RecentSearch, idx: number) => (
