...
export function DatePickerScreen({ navigate }: DatePickerScreenProps) {
    ...
    if (state.tripType === 'roundTrip' && selectingReturn) {
      ...
      if (state.tripType === 'roundTrip') {
        if (date > selRet) {
          ...
          setSelRet(d.toISOString().slice(0, 10));
        }
  ...
  function handleConfirm() {
    ...
    if (state.tripType === 'roundTrip') {
      dispatch({ type: 'SET_RETURN_DATE', payload: selRet });
    }
    navigate('search');
  }
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col" aria-label={t.datePicker.heading}>
            ...
            <div className="grid grid-cols-7 gap-1">
              {m.days.map((day) => {
                ...
                return (
                  <button
                    key={day.date}
                    className={`h-10 w-full rounded text-xs font-medium transition-colors ${
                      day.disabled
      ...
      <div className="p-4 border-t border-gray-200">
        <p className="text-sm text-gray-700" data-testid="selected-departure-label">
          {t.datePicker.departureLabel.replace('{date}', selDep)}
        </p>
        {state.tripType === 'roundTrip' && (
          <p className="text-sm text-gray-700 mt-1" data-testid="selected-return-label">
            {t.datePicker.returnLabel.replace('{date}', selRet)}
          </p>
        )}
        <button
          className="w-full mt-3 py-3 bg-red-600 text-white rounded-lg font-semibold text-sm"
          onClick={handleConfirm}
          aria-label={t.datePicker.confirm}
