...
export function DoneSuccessScreen({ navigate }: DoneSuccessScreenProps) {
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: t.done.bookingCode,
          text: `${t.done.bookingCode}: ${result.bookingCode}\n${t.done.amountCharged}: ${formatPrice(result.amount)}`,
        });
      } catch {
  ...
  return (
    <div className="p-4 flex flex-col items-center gap-4">
      ...
      <p className="text-2xl font-bold text-red-600" data-testid="amount-display" aria-label={t.done.amountCharged}>
      ...
      <div className="w-full border border-gray-200 rounded-lg p-4" data-testid="booking-details">
        ...
        {result.transactionId && (
          <div className="flex justify-between mb-2">
            ...
            <span className="text-sm" data-testid="transaction-id">{result.transactionId}</span>
          </div>
        )}
        {result.vatRequested && (
          <p className="text-xs text-gray-500" data-testid="vat-notice">{t.done.vatNotice}</p>
        )}
      </div>
      ...
      <button
        className={`w-full py-2 border border-gray-300 rounded-lg text-sm font-medium ${
          canShare ? 'text-gray-700 hover:bg-gray-50' : 'text-gray-400 cursor-not-allowed'
        }`}
        disabled={!canShare}
        onClick={handleShare}
        aria-label={t.done.share}
      ...
      <div className="flex gap-3 w-full">
        <button
          ...
          aria-label={t.done.bookAnother}
          data-testid="cta-book-another"
        >
          {t.done.bookAnother}
        </button>
        <button
