...
export function DonePartialScreen({ navigate }: DonePartialScreenProps) {
  ...
  return (
    <div className="p-4 flex flex-col items-center gap-4">
      <span className="text-6xl text-amber-500" aria-hidden="true">&#9888;</span>
      <h1 className="text-2xl font-bold text-gray-900">{t.done.partialHeading}</h1>
      <p className="text-2xl font-bold text-gray-700" data-testid="amount-display" aria-label={t.done.amountCharged}>
        {formatPrice(result.amount)}
      </p>

      <div className="w-full border border-gray-200 rounded-lg p-4" data-testid="booking-details">
        <div className="flex justify-between mb-2">
          <span className="text-sm text-gray-700">{t.done.outboundBookingCode}</span>
          <span className="font-bold text-sm" data-testid="outbound-booking-code">{result.outboundBookingCode ?? result.bookingCode}</span>
        </div>
        <p className="text-sm text-gray-700" data-testid="partial-explanation">
          {t.done.partialExplanation}
        </p>
      </div>
