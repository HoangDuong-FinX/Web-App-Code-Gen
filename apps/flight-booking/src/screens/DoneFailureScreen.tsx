...
export function DoneFailureScreen({ navigate }: DoneFailureScreenProps) {
  ...
  function handleRetry() { navigate('checkout'); }

  return (
    <div className="p-4 flex flex-col items-center gap-4">
      <span className="text-6xl text-red-500" aria-hidden="true">{String.fromCharCode(10008)}</span>
      <h1 className="text-2xl font-bold text-gray-900">{t.done.failureHeading}</h1>
      <p className="text-2xl font-bold text-gray-700" data-testid="amount-display" aria-label={t.done.amountAttempted}>{formatPrice(result.amount)}</p>
      <div className="w-full border border-gray-200 rounded-lg p-4" data-testid="booking-details">
        ...
        {result.failureReason && <p className="text-red-600 text-sm" data-testid="failure-reason">{t.done.failureReason}: {result.failureReason}</p>}
      </div>
      ...
      <div className="flex gap-3 w-full">
        ...
        <button className="flex-1 py-3 bg-red-600 text-white rounded-lg font-semibold text-sm hover:bg-red-700 transition-colors" onClick={handleRetry} aria-label={t.done.retry} data-testid="cta-retry">{t.done.retry}</button>
      </div>
    </div>
  );
}
