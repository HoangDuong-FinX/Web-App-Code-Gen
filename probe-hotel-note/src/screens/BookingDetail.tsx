...
export const BookingDetail: React.FC<BookingDetailProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        onSaveFailed(result.message || 'Failed to save note. Please try again.');
      }
    } catch (error) {
      onSaveFailed('Network error: Unable to save note');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const isSaveDisabled = isSaving || !isNoteValid;
  ...
  return (
    <div style={styles.container}>
      ...
      <Button
        ...
        aria-label={t('booking-detail.save-aria')}
      >
        {isSaving ? t('booking-detail.saving') : t('booking-detail.save-button')}
      </Button>
    </div>
  );
};
